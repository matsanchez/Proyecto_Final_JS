const words = ['alias','arrestar','explosion','rompecabezas','brillar','australiano','naturalista','cuchara','parametros','nativo','ladron','retirar','escondida','television','javascript','html','respirar','choque','resonancia','trozos','rehenes','medico','cadena','pastel','zorro','cuadro','astronomia','mineros','estatua','conejo','paquete','malestar','natacion','excepcional','emperador','rapero','equipamiento','educativo','guionista','motocicleta','gusano','lastimar','inmune','covid','maestro']
...
DOMplay_game.addEventListener('click', init);
...
document.addEventListener('click', ((e) => {
    if (e.target && e.target.matches('#letters button')) {
        ...
        if (letter_wrongs == 7) {
            ...
            gameOver();
        } else if (letter_corrects == wordRandom.length) {
            Swal.fire({
                title: 'GANASTE!',
                width: 600,
                padding: '3em',
